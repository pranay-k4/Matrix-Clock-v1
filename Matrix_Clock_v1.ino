...
#define BTN_MODE 32 // Switch display mode
#define BTN_BRIGHT 33 // Changes Brightness
...
void drawMessage() {
  ...
  dma_display->setTextSize(1);
  dma_display->setCursor(scrollX, 12);
  
  dma_display->setTextColor(
    dma_display->color565(255, 100, 0)
    );

   dma_display->print(customQuote);
  scrollX--;

  int textWidth = customQuote.length() * 6;
